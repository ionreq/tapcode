...
with morse code there is a problem: the pure bit efficiency is very low, over 8 bit, but:
in practice, a dit would correspond to a tap in the tap code, so for comparing efficiency in reality, one would have to halve the bit efficiency.
